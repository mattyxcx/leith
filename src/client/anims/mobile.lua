...
local main = script.Parent.Parent.mobile.Main
...
local createCell = function(tbl)
    local cell = sv.cat.mob_temp.cell:Clone()
...
local fixInv = function()
    ...
    for _,v in ipairs(sv.players.LocalPlayer.Character.Humanoid:GetAccessories()) do
        local cell = sv.cat.mob_temp.inv:Clone()
...
local createPage = function(name)
    local page = sv.cat.mob_temp.scroll:Clone()
    page.Name = name
    page.Visible = true
            ...
            btn.Activated:Connect(function()
                ...
                elseif par == "search" then
                    ...
                    elseif func == "toggle" then
                        main.Search.TextBox.Visible = not main.Search.TextBox.Visible
                        main.Search.search_action.Visible = not main.Search.search_action.Visible
                        btn.Icon.Image = (main.Search.TextBox.Visible and "rbxassetid://12579503291") or "rbxassetid://12330982380"
                        if main.Search.TextBox.Visible then main.Search.TextBox:CaptureFocus() else main.Search.TextBox:ReleaseFocus() end
                    end
                end
            end)
        end
...
local createIcon = function()
    local toggle = function()
        if cache.visible then
            ...
            main:TweenPosition(UDim2.new(1.425,0,0.5,0),"Out","Quart",0.3,true)
            pcall(function() sv.players.LocalPlayer.Character.Humanoid.JumpPower = 50 end)
        else
            transition(main.UIScale,"show")
            main:TweenPosition(UDim2.new(0.975,0,0.5,0),"Out","Quart",0.3,true)
            pcall(function() sv.players.LocalPlayer.Character.Humanoid.JumpPower = 0 end)
        end
